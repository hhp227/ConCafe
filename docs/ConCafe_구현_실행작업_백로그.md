...
- 작업:
  ...
  5. 판매 상태 토글 UI와 편집/삭제/추가 CTA 자리 표시 연결
- 메모:
  - 현재 단계는 읽기 전용 운영 화면 + 로컬 UI 상태 토글까지 구현한다.
  - 실제 메뉴/굿즈 CRUD 저장, 서버 동기화, 이미지 업로드는 후속 단계에서 구현한다.
- AC:
  ...
  - 검색/카테고리/판매 상태 토글이 플랫폼별로 동일한 UI 규칙으로 반영된다.
