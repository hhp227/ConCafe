...
## 최근 반영 (2026-03-19)
- 이미지 입력 폼 업로드 경로를 KMP `UploadImageUseCase` 기준으로 통일했다.
- 업로드 정책을 `1MB 초과 시 1MB 미만으로 압축`으로 적용했다.
...
- Firestore 컬렉션 다이어그램/통합 기획서 기준으로 Claim 책임을 정리했다:  
  `castClaims`(팬관리 신청 -> 운영자 승인), `cafeOwnerClaims`/`cafeRegistrationClaims`(Admin 승인).
...
- 작업:
  ...
  2. 리뷰 작성 로그인 조건 및 방문 인증 배지 정책 확정
...
### B-06. DataSource 인터페이스 책임 분리
- 우선순위: P0
- 상태: DOING
- 산출물: DataSource 인터페이스 책임표 + Repository 의존성 정리 내역
- 작업:
  1. 사용자 조회/수정 계약을 `AuthDataSource` 메서드 기반으로 통일
  2. 사용자 컬렉션 내부 상태는 구현체(private)에서만 관리
  3. Repository에서 데이터소스 내부 컬렉션 직접 접근 제거
  4. DI 바인딩에서 제거된 인터페이스(`UserDataSource`) 참조 정리
- AC:
  - Repository가 데이터소스 내부 리스트/맵 구조에 직접 의존하지 않는다.
  - 사용자 관련 읽기/쓰기 경로가 인터페이스 메서드로 일관된다.
  - mock -> Firestore 구현 전환 시 Repository 코드 변경이 최소화된다.
...
- 작업:
  ...
  2. 마이 페이지 진입 시 비로그인이면 게스트 화면을 보여주고, 로그인 버튼으로 로그인 화면에 진입
...
- 작업:
  ...
  2. 플랫폼별 `SettingsScreen`/`SettingsView` UI 구현
  3. 앱 버전, 계정 관리, 알림 설정, 개인정보 처리방침 항목 배치
  4. `SignOut` 액션 진입점 연결
  5. 뒤로가기 시 `MyInfo`로 복귀 규칙 확인
  6. 계정 관리 / 알림 설정 상세 화면 push 이동 연결
- 메모:
  - 현재 범위에서는 설정값 저장/동기화 기능을 만들지 않는다.
  - 계정 관리와 알림 설정은 상세 UI까지 구현되었지만 실제 서버 저장은 후속 단계다.

### C-01-2. Presentation 서버연결 전 정리
- 우선순위: P0
- 상태: DOING
- 산출물: mock 제거 전 Presentation 하드닝 체크리스트 완료
- 작업:
  1. 로그인 가드 분산 처리 구간을 공통 규칙으로 정리
  2. `pendingRoute/pendingAction` 복귀 UX를 Navigation 계층 기준으로 통일
  3. 설정 계정 관리 / 비밀번호 변경의 로컬 성공 메시지를 실제 저장 플로우로 교체
  4. 리뷰 이미지 업로드 placeholder 제거
  5. 캐스트 프로필 이미지 / 카페 대표 이미지 / 갤러리 이미지 업로드 placeholder 제거
  6. 팬관리 / 배너 / 출근표 / 운영 화면의 `다음 단계` 안내 액션을 실제 라우트 또는 명시적 제외 범위로 정리
  7. iOS ViewModel 취소 핸들(`Task` / `WatchHandle`) 관리 패턴을 공통 자료구조로 통일할지 검토하고 리팩터링 범위를 확정
- AC:
  - Presentation 계층에 `로컬 상태만 반영됩니다`, `다음 단계에서 연결됩니다` 같은 임시 성공 처리 문구가 서버 필수 플로우에서 제거된다.
  - 보호 액션은 동일한 로그인 가드 정책으로 동작한다.
  - 서버 연결 시 ViewModel 교체 없이 UseCase 응답만 바꿔도 동작 가능한 수준으로 정리된다.
  - iOS 취소 핸들 관리 패턴의 목표안과 적용 범위가 문서로 정리된다.
- 진행 메모:
  - 4, 5번(리뷰/캐스트/카페 이미지 업로드 placeholder 제거)은 완료했고, 공통 업로드 use case로 정리했다.
...
- 우선순위: P1
- 상태: DONE
- 산출물: 홈 화면(인기 캐스트/근처 카페/생일/공지)
- 작업:
  1. 섹션별 데이터 소스 연결
  2. 로딩/에러/빈 상태 처리
  3. 카드 클릭 내비게이션 연결
  4. 데이터 부족 시 섹션 숨김/대체 카드 정책 반영
  5. 인기 캐스트 / 근처 카페 페이지네이션 연결
- AC:
  - 4개 섹션이 모두 렌더링된다.
  - 섹션별 에러가 화면 전체를 망가뜨리지 않는다.
  - 생일/공지 데이터가 없을 때도 홈 레이아웃이 깨지지 않는다.
  - 인기 캐스트는 10개 단위로 추가 로드되고 현재 mock 기준 최대 50개까지만 노출된다.
...
- 작업:
  ...
  2. 내부 탭(카페/메이드) 전환
  3. 페이징/무한 스크롤 처리
  4. 카페/메이드 전용 page use case 분리
- AC:
  - 필터 조합 변경 시 결과가 일관되게 갱신된다.
  - 스크롤 성능 저하 없이 목록이 확장된다.
  - 카페/메이드 모두 페이지 크기 15개 기준으로 하단 도달 시 자동 추가 로드된다.
...
- 진행 메모:
  1. 역할별 3번째 탭(`체크인/팬관리/카페관리/운영관리`) 치환은 구현 반영 완료
  2. 세션 변경 스트림 구독 기준으로 로그인/로그아웃 시 탭 재구성 반영 완료
...
- 메모:
  ...
  - 프로필 사진/갤러리 업로드는 mock storage 경유로 연결 완료, 외부 SNS 링크 CRUD는 후속 단계다.
- AC:
...
- 산출물: 리뷰 작성/조회
- 작업:
  1. 로그인 사용자 작성 허용(방문 인증은 배지 노출로 분리)
  2. 별점/내용/이미지 업로드(mock storage 경유)
...
- AC:
  ...
  - 방문 인증 여부는 작성 차단 조건이 아니라 리뷰/작성 화면 배지 노출 정보로만 사용된다.
...
- AC:
  - 비로그인 접근 시 게스트 화면이 노출되고 로그인 버튼으로 로그인 화면에 진입된다.
  - 로그인 성공 후 마이 페이지 사용자 상태가 즉시 반영된다.
  - 사용자 기준 개인 데이터만 노출된다.
...
- 작업:
  ...
  4. 랭킹 피드 shared use case 연결
  5. mock 데이터소스 기준 최대 노출 50개 제한
- AC:
  - 사용자는 랭킹 탭에서 카페/메이드 순위를 조회할 수 있다.
  - 점수식 확정 전에도 정렬 기준이 문서화되어 있다.
  - Compose/iOS 양 플랫폼이 동일한 shared `RankingFeed`를 사용한다.

### C-12. 리뷰 정책 정리
- 우선순위: P0
- 상태: DONE
- 산출물: 현재 MVP 리뷰 정책과 Presentation 노출 규칙 정리
- 작업:
  1. 리뷰 작성 조건을 `로그인 사용자` 기준으로 고정
  2. 방문 인증 여부는 작성 가능 조건이 아니라 배지 노출 정보로만 사용
  3. 리뷰 작성 화면의 방문 인증 배지와 리뷰 아이템 배지 노출 기준을 동일화
  4. 리뷰 이미지 업로드를 shared mock storage 경로로 연결(Firebase Storage 연동은 후속)
- AC:
  - 로그인 사용자는 방문 인증 없이도 리뷰 작성 화면에 진입하고 저장할 수 있다.
  - 방문 인증 사용자는 리뷰 작성 화면과 리뷰 아이템에서만 인증 배지가 노출된다.
  - 비로그인 사용자는 카페 상세의 리뷰 작성 CTA가 노출되지 않는다.
...
- 작업:
  1. 리뷰 생성 시 로그인 사용자 검증
  2. 방문 인증 배지 계산 규칙 정리
  3. 실패 코드 표준화
- AC:
  - 비로그인 리뷰 생성이 100% 차단된다.
  - 방문 인증 배지가 실제 방문 인증 데이터와 일치한다.
...
- 작업:
  ...
  2. 리뷰 작성(로그인 필요) 및 방문 인증 배지 계산 테스트
- AC:
