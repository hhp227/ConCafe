...
## 구현 기준 스냅샷 (2026-03-22)

- 런타임 데이터 계층은 Firestore/Firebase Auth/Firebase Storage REST + Ktor 경로를 사용한다.
- `MockConCafeDataSource`는 현재 테스트 코드(`shared/src/commonTest`) 전용이다.
- 세션 복원은 Android/iOS/Desktop 공통으로 `restoreSession` + `observeCurrentUser` 흐름을 사용한다.
...
기존 카페 운영자 신청과 신규 카페 등록 신청은 explicit event로 카페관리 화면에 즉시 반영한다.
...
현재 앱 구현에서는 `신규 카페 등록`이 별도 전용 화면이 아니라 `카페 정보 수정` 폼의 등록 모드를 재사용한다.
등록 요청은 `cafeRegistrationClaims`에 저장되고, Admin 승인 시 실제 `cafes/{cafeId}`와 owner 연결이 생성된다.
...
현재 MVP 앱 구현 범위에서는 카페 대시보드에서 `메뉴&굿즈 관리` 화면으로 진입할 수 있으며,
Android Compose와 iOS SwiftUI에 동일한 상태 구조의 운영 UI를 제공한다.
현재 구현 범위에는 목록 조회, 탭 전환, 카테고리 필터, 검색, 메뉴/굿즈 추가, 기존 항목 수정, 삭제 확인 UX 후 실제 삭제가 포함된다.
메뉴 판매 상태는 공용 도메인 모델의 `CafeMenu.isAvailable` 필드에 저장하며,
굿즈 판매 상태는 `Goods.stock` 기반으로 관리한다.
목록 갱신은 화면 재진입 refresh가 아니라 공용 `observeCafeDetail` 스트림으로 즉시 반영한다.
같은 운영 범위에서 `캐스트 프로필 추가/수정` 공용 화면도 Android Compose와 iOS SwiftUI에 구현되었다.
수정 모드 초기값은 `GetCastDetailUseCase`로 로드하고, 저장은 `UpsertCastUseCase`를 통해 공유 데이터 계층에 반영한다.
현재 캐스트 저장 범위는 이름, 컨셉 역할, 생일, 소개, 근무 요일이며, 근무 요일은 `castSchedules` 생성/갱신 기준으로 관리한다.
이미지 업로드는 현재 Firebase Storage 업로드 경로로 연결되어 있다.
운영 도메인별 영속 동기화 범위(예: 외부 링크/고급 운영 통계)는 후속 운영 고도화 단계에서 확장한다.
카페 대시보드에는 외부 링크 입력 시트와 카드 리스트 섹션도 추가되었으며,
링크가 있을 때만 섹션을 노출하고 카드 탭 시 앱 내부 외부 링크 화면(WebView)으로 이동한다.
현재 외부 링크 섹션은 대시보드 화면 상태 기준 구현이며 shared 영속 저장은 후속 범위다.
...
→ 소속 카페 운영자가 `카페 관리 대시보드 > 캐스트 관리 섹션`에서 승인 / 반려

→ 승인되면 `cafes/{cafeId}/casts/{castId}.linkedUserId = userId`로 연결
...
- 캐스트는 회원가입 자체는 완료할 수 있다.
- 기존 캐스트 프로필과의 연결은 소속 카페 운영자의 승인이 있어야 한다.
- 가입 시 선택한 `소속 카페`는 계정에 연결 대기 정보로 저장한다.
- 가입 후 `팬관리`에서 `캐스트 프로필 연결` 전용 UI를 통해 신청 상태를 확인하고 연결 요청을 보낼 수 있다.
- 캐스트는 운영자용 카페 대시보드로 진입하지 않는다.
- 카페 운영자는 `카페 관리 대시보드 > 캐스트 관리 섹션` 상단의 `프로필 연결 요청` 카드에서 신청을 검토한다.
...
현재 MVP 구현 범위에서는 `캐스트 프로필 추가/수정` 공용 화면이 먼저 구현되어 있으며,
캐스트 상세 데이터를 불러와 수정하고 공유 데이터 계층에 upsert할 수 있다.
프로필 이미지/갤러리 업로드는 Firebase Storage 경로를 사용한다.
외부 SNS 링크 영속 CRUD는 후속 범위다.
...
### 상세 화면 접근 정책 (2026-03-19 업데이트)
- 메인 탭(`홈`, `탐색`, `체크인`, `랭킹`, `내 정보`)에서 카페 상세/캐스트 상세로 이동할 때 로그인 여부를 먼저 확인한다.
- 비로그인 상태에서는 상세 화면 네비게이션을 차단하고 로그인 유도 Dialog/Alert를 표시한다.
- 사용자 선택지는 `로그인`(SignIn 이동) 또는 `취소`로 통일한다.
...
현재 앱 구현에서는 Compose/iOS 랭킹 화면이 shared `GetRankingFeedUseCase`를 통해 공통 `RankingFeed`를 조회한다.
현재 런타임 데이터 계층 기준 랭킹 노출은 페이지네이션 정책으로 관리한다.
...
- 주간 캘린더
- 캐스트별 근무 시간 표시
- 수정 UI는 `근무 / 휴무 / 휴가` segmented 선택과 시작/종료 시간 입력을 제공한다.
- 일자별 수정 저장은 shared KMP `UpdateCastScheduleUseCase`를 통해 즉시 반영된다.
- 저장 후 `ScheduleManagementEvent.Updated` explicit event를 기준으로 Compose/iOS UI가 재조회된다.
...
- 현재 구현에서는 홈 피드 조회 시 종료된 활성 배너를 정리하고, 빈 슬롯이 생기면 `SCHEDULED` 배너를 자동 승격한다.
- 관리자는 전체 배너를 관리할 수 있고, 카페 운영자는 본인이 운영 권한을 가진 카페와 연결된 배너만 관리할 수 있다.
- 배너 클릭 시 카페 상세, 이벤트 상세, 공지, 외부 프로모션 링크 등으로 연결할 수 있다.
- 외부 프로모션 링크는 외부 브라우저가 아니라 앱 내부 외부 링크 화면(WebView)으로 연다.
...
3. 제목 / 서브 문구 입력
4. 연결 대상 설정
5. 노출 기간 슬라이더 선택 (1일 ~ 10일)
6. 저장 후 상태 자동 계산

현재 구현 기준 연결 대상 UX는 다음과 같다.
- `카페 상세`: 운영자의 현재 카페 컨텍스트를 기본값으로 사용
- `공지사항`, `이벤트 상세`: 관리자는 전체 카페 선택 가능, 운영자는 본인 카페 기준으로만 선택
- `외부 링크`: URL 직접 입력
...
- 현재 구현 범위는 `Settings` 스크린과 상세 화면 push 연결까지 포함한다.
- 포함 항목: 계정 관리, 알림 설정, 개인정보 처리방침, 앱 버전 정보, SignOut
- `계정 관리`는 기본 프로필 수정 폼과 비밀번호 변경/캐스트 정보 수정 화면 이동, 회원탈퇴 확인 입력 UX를 포함한다.
- `알림 설정`은 알림 유형별 토글 UI와 조용한 시간 선택 UI를 포함한다.
- `개인정보 처리방침`은 인앱 외부 링크 화면으로 이동하며 현재 임시 URL은 `http://www.concafe.app`다.
- 설정 화면은 `SignOut`, 계정 관리 상세 진입, 알림 설정 상세 진입, 개인정보 처리방침 이동을 제공한다.
- 회원탈퇴는 재인증(비밀번호 확인) 후 삭제 플로우를 사용한다.
- 계정 관리/알림 설정의 일부 항목은 UI 우선 단계이며 세부 서버 동기화 확장은 후속 단계다.
Mock 제거 전에는 이 구간들의 성공/실패 UX를 서버 응답 기준으로 먼저 치환해야 한다.

------------------------------------------------------------------------

## 📝 리뷰 정책

- 리뷰 작성은 로그인한 사용자라면 가능하다.
- 방문 인증은 리뷰 작성의 필수 선행조건이 아니다.
- 대신 방문 인증 이력이 있는 사용자는 리뷰 작성 화면과 리뷰 아이템에서 `방문 인증` 배지를 노출한다.
- 리뷰 이미지 업로드는 현재 Firebase Storage 업로드 경로가 연결되어 있다.
