...
Android Compose와 iOS SwiftUI에 동일한 상태 구조의 읽기 전용 운영 UI를 우선 제공한다.
이 단계에서 제공하는 범위는 목록 조회, 탭 전환, 카테고리 필터, 검색, 판매 상태 토글 UI이며,
실제 CRUD 저장 및 서버 동기화는 후속 운영 고도화 단계에서 이어서 구현한다.
