...
현재 앱 구현 범위에서는 `운영관리` 대시보드가 실제 shared mock의 `cafeOwnerClaims`, `cafeRegistrationClaims`를 읽고 승인 / 반려를 처리한다.
...
수정 모드 초기값은 `GetCastDetailUseCase`로 로드하고, 저장은 `UpsertCastUseCase`를 통해 shared mock 데이터에 반영한다.
현재 캐스트 저장 범위는 이름, 컨셉 역할, 생일, 소개, 근무 요일이며, 근무 요일은 `castSchedules` 생성/갱신 기준으로 관리한다.
실제 서버 동기화와 이미지 업로드는 후속 운영 고도화 단계에서 이어서 구현한다.
...
캐스트 상세 데이터를 불러와 수정하고 shared mock 저장소에 upsert할 수 있다.
프로필 이미지/갤러리 업로드와 외부 SNS 링크 관리는 후속 범위다.
...
현재 mock 데이터소스 기준 랭킹 최대 노출 수는 메이드/카페 각각 50개다.
